...
bool tokenIsNumber(const Token* t) {
...
bool tokenIsParenthesis(const Token* t) {
	return t->type == parenthesis;
}

float tokenGetValue(const Token* t) {
	assert( tokenIsNumber(t) );
	return t->value.number;
}

char tokenGetOperatorSymbol(const Token* t) {
	assert(tokenIsOperator(t) );
	return t->value.symbol;
}

char tokenGetParenthesisSymbol(const Token* t) {
	assert( tokenIsParenthesis(t) );
	return t->value.symbol;
}

int tokenGetOperatorPriority(const Token* t){
	assert( tokenIsOperator(t) );
...
bool tokenOperatorIsLeftAssociative(const Token* t){
	assert( tokenIsOperator(t) );
	switch( t->value.symbol ) {
		case '+':
		case '-':
...
void tokenDump(FILE* f, const Token* t) {
	if (t->type == number)
